...
class Aligner:
  ...
  def _create_info_files(self, max_offset):
    ...
    field_info = deepcopy(src_info)
    vec_info["data_type"] = "float32"
    field_info["data_type"] = "int16"
    ...
    for i in range(len(scales)):
      ...
      cv(self.x_field_ng_paths[i], info=field_info).commit_info()
    #  cv(self.x_field_ng_paths[i], info=vec_info).commit_info()
      cv(self.y_field_ng_paths[i], info=field_info).commit_info()
  ...
  def compute_residual_patch(self, source_z, target_z, out_patch_bbox, mip):
    ...
    field, residuals, encodings, cum_residuals = self.net.process(src_patch, tgt_patch, mip, crop=self.crop_amount)
  ...
  def save_vector_patch(self, flow, x_path, y_path, z, bbox, mip):
    ...
    x_res = np.int16(x_res * 8)
    y_res = np.int16(y_res * 8)
    x_range = bbox.x_range(mip=mip)
    y_range = bbox.y_range(mip=mip)
