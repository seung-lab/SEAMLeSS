...
class DConv(nn.Module):
    def __init__(self, infm, outfm, k, padding, dilation=1, groups=1, f=nn.ReLU(inplace=True)):
        ...
        weights = torch.zeros((outfm, infm, k, k)).normal_(0, 0.001)
        for i in range(infm):
...
class DG(nn.Module):
    def __init__(self, k=3, f=nn.ReLU(), t=1):
        ...
        self.f = f
        fm = 32 * (t+1)
        self.conv1 = nn.Conv2d(t+1, fm, k, padding=2, groups=t+1)
        self.conv2 = nn.Conv2d(fm, fm, k, padding=2)
...
class Pyramid(nn.Module):
    ...
    def __init__(self, size, dim, skip, k, dilate=False, amp=False, unet=False):
        ...
        self.skip = skip
        self.size = size
        self.mlist = nn.ModuleList([DG(k=k) for level in range(size)])
...
class PyramidTransformer(nn.Module):
    def __init__(self, size=4, dim=192, skip=0, k=5, dilate=False, amp=False, unet=False):
