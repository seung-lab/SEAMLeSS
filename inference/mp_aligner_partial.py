...
class Aligner:
  ...
  def checkpoint_write(self, f_list, arg_list, z, cv_path, final=True):
  ...
  def read_pre_field(self, cv, z, dic, key, bbox, start_coor, chunk_size, mip):
      start = time()
      x_range = bbox.x_range(mip=mip)
      y_range = bbox.y_range(mip=mip)
      start_x = (x_range[0] -start_coor[0]) // chunk_size
      end_x = (x_range[1] -start_coor[0]) // chunk_size
      start_y = (y_range[0] -start_coor[1]) // chunk_size
      end_y = (y_range[1] -start_coor[1]) // chunk_size
      data = cv[mip][start_x:end_x, start_y:end_y, z]
      data = torch.from_numpy(data)
      data = torch.squeeze(data, 2)
      dic[key] = data
      end = time()
      print("----------finish read pre_field using", end-start)


  def write_pre_field(self, args):
      print("........................ in write_pre_field")
      start=time()
      cv = args[0]
      z = args[1]
      dic = args[2]
      ...
      y_range = bbox.y_range(mip=mip)
      add_mip = 0
      print("<<<<<<<<<<<<<<<<<<<<<<<<<--x_range, y_range in write pre field", x_range, y_range)
      print("data shape is", data.shape)
      print("<<<<<<<<<< start coor is", start_coor)
      data = data.numpy()
      x_range = bbox.x_range(mip=mip)
      y_range = bbox.y_range(mip=mip)
      start_x = (x_range[0] -start_coor[0]) // chunk_size
      ...
      start_y = (y_range[0] -start_coor[1]) // chunk_size
      end_y = (y_range[1] -start_coor[1]) // chunk_size
      cv[mip][start_x:end_x, start_y:end_y, z] = data
      del dic[key]
      end = time()
      print("--------finish write pre field using", end-start, "key is", key)
  ...
  def new_align_task(self, block_start, block_stop, start_z,
                     ...
                     extra_off, pre_field_cv, src_mask_cv=None, src_mask_mip=0,
                     src_mask_val=0, super_chunk_len=1000, overlap_chunks=0):
      #print("---------------------------->> load image")
      batch = []
      #for i in z_range:
      batch.append(tasks.NewAlignTaskP(src, qu, dst, s_field, vvote_field, chunk_grid, mip, pad,
                                      block_start, block_stop, start_z, chunk_size,
                                      model_lookup, finish_dir, timeout,
                                      extra_off, pre_field_cv,
                                      src_mask_cv, src_mask_mip,
                                      src_mask_val, super_chunk_len,
                                      overlap_chunks))
      return batch
  ...
  def new_align(self, src, dst, s_field, vvote_field, chunk_grid, mip, pad, block_start,
                ...
                timeout, extra_off, pre_field_cv, src_mask_cv=None, src_mask_mip=0,
                src_mask_val=0, super_chunk_len=1000,
                overlap_chunks=0):
      print('-------------- super_chunk_len is ', super_chunk_len)
      model_lookup={}
      tgt_radius_lookup = {}
      vvote_lookup = {}
      skip_list = []
      ...
      start_chunk = deepcopy(chunk_grid[0])
      start_chunk.crop(pad, mip)
      x_coor = start_chunk.x_range(mip=mip)[0]
      y_coor = start_chunk.y_range(mip=mip)[0]
      start_coor = (x_coor, y_coor)
      final = False
      while(start_index<len(block_range)):
          for i in range(len(chunk_grid)):
              ...
              serial_chunk= self.crop_chunk(chunk, mip, head_crop_len, end_crop_len, 0, 0)

              print("----------------- start index is", start_index, "chunk id is", i)
              if start_index==0 and ((start_z in starter_range) or (start_z==block_start)):
                  image_list = self.process_super_chunk_serial(src, block_start,
                                                              starter_range,
                                                              start_z,
                                                              dst, model_lookup,
                                                              chunk, mip, pad, chunk_size,
                                                              serial_chunk, finish_dir,
                                                              extra_off,
                                                              head_crop_len,
                                                              end_crop_len,
                                                              final,
                  ...
                  if start_index<max_radius:
                      num_in_serial = max_radius - start_index
                      if num_in_serial > 1:
                      ...
                      for z in block_range[start_index-max_radius:start_index]:
                          p = Process(target=self.mp_load, args=(dic, z, dst,
                                                                z, chunk,
                                                                mip))
                          p.start()
              ...
              self.process_super_chunk_vvote(src, block_range, max_radius,
                                             super_chunk_len,
                                             dst,
                                             model_lookup, tgt_radius_lookup,
                                             image_list, serial_chunk, start_index,
                                             mip, pad, extra_off,
                                             chunk_size, vvote_field,
                                             finish_dir,
                                             pre_field_cv, start_coor,
                                             head_crop, end_crop, final,
                                             mask_cv=src_mask_cv,
                                             mask_mip=src_mask_mip,
                                             mask_val=src_mask_val)
              self.join_with_timeout(self.p_list, self.IO_timeout)
          start_index+=super_chunk_len
      self.manager = None
      self.dic = None
  ...
  def stitch_get_field_task_generator(self, qu, param_lookup, bs, be,
                                      ...
                                      timeout, extra_off, super_chunk_len,
                                      softmin_temp, blur_sigma):
      batch = []
      batch.append(tasks.StitchGetFieldP(qu, param_lookup, bs, be, src_cv, tgt_cv,
                                         prev_field_cv, bfield_cv,
                                         tmp_img_cv, tmp_vvote_field_cv,
                                         pre_field_profile_cv, tmp_profile_cv,
                                         mip, pad, bbox, start_z, finish_dir,
  ...
  def get_stitch_field_task(self, param_lookup, bs, be, src_cv, tgt_cv, prev_field_cv,
      ...
      self.get_stitch_field(model_lookup, src_cv, tgt_cv, prev_field_cv,
                            ...
                            mip, bbox, chunk_size, pad, super_chunk_len,
  ...
  def get_stitch_field(self, model_lookup, src_cv, tgt_cv, prev_field_cv,
                       ...
                       finish_dir, extra_off, mip, chunk_grid, chunk_size,
                       pad, super_chunk_len,
      ...
      while(start_index<len(compute_field_range)):
          for i, bbox in enumerate(chunk_grid):
              if(len(chunk_grid) == 1):
                  head_crop = False
                  end_crop = False
                  final = True
                  ...
                  if i == 0:
                      ...
                      head_crop = True
                      end_crop = True
              chunk, dst_fields = self.stitch_calc_field(model_lookup, src_cv, tgt_cv,
                                                         prev_field_cv,
                                                         pre_field_profile_cv,
                                                  tmp_img_cv, tmp_vvote_field_cv,
                                                  tmp_profile_cv,
                                                  overlap_copy, compute_field_range,
                                                  start_index, finish_dir, extra_off, mip,
                                                  bbox, chunk_size, pad, head_crop,
                                                  end_crop, final, start_coor,
                                                  softmin_temp=softmin_temp,
                                                  blur_sigma=blur_sigma)
              print("-------- before crop field ")
              for i in dst_fields:
                  print(" field shape", i.shape)
              self.crop_field_list(head_crop, end_crop, dst_fields)
              print("-------- after crop field ")
              for i in dst_fields:
                  print(" field shape", i.shape)
              ...
              chunk.crop(extra_off+pad, mip=mip)
              self.mp_store_field([dic, bc, bfield_cv, overlap_copy[-1], chunk, mip,
                            chunk_size])
              self.join_with_timeout(self.p_list, self.IO_timeout)
          start_index+=super_chunk_len
  ...
  def stitch_calc_field(self, model_lookup, src_cv, tgt_cv, prev_field_cv,
                        pre_field_profile_cv, tmp_img_cv, tmp_vvote_field_cv,
                        tmp_profile_cv, overlap_copy,
                        compute_field_range, start_index, finish_dir,
                        extra_off, mip, bbox, chunk_size,
                        pad, head_crop, end_crop, final, start_coor,
                        softmin_temp=None, blur_sigma=None):
      chunk = deepcopy(bbox)
      ...
      start_z = compute_field_range[start_index]
      ...
      for z in load_slice_range:
          if z in overlap_copy:
              local_img_cv = tgt_cv
              local_field_cv = pre_field_profile_cv
          elif z in compute_field_range:
              local_img_cv = tmp_img_cv
              local_field_cv = tmp_profile_cv
          ...
          pf = Process(target=self.read_pre_field, args=(local_field_cv, z, dic,
                                                        pre_field_key,
                                                        nonpad_chunk,
                                                        start_coor,
                                                        chunk_size, mip))
      ...
      f_list = [self.write_pre_field, self.mp_store_img, self.mp_store_field]
      for i, z in enumerate(compute_range):
          ...
          x_len = src_image.shape[-2] - 2*extra_off
          self.crop_image_list(head_crop, end_crop, image_list, x_len)
          self.crop_prefield_list(head_crop, end_crop, pre_field)
          ...
          field_key = "field" + str(z)
          #if end_crop:
          #    dst_field = dst_field[:, head_crop_len:-end_crop_len,...]
          #else:
          #    dst_field = dst_field[:, head_crop_len:,...]
          dst_field = dst_field[:, pad+head_crop_len:-(pad+end_crop_len),
                                     pad:-pad,:]

          dic[field_key] = dst_field
          img_key = "image" + str(z)
          dic[img_key] = image[...,pad+head_crop_len:-(pad+end_crop_len),
                               pad:-pad]

          pre_field_key = self.pre_field_path+str(mip)+"/"+str(z)
          tmp_pre_field = pre_field[-1]

          if head_crop:
              tmp_pre_field = tmp_pre_field[1:,...]
          if end_crop:
              tmp_pre_field = tmp_pre_field[:-1, ...]

          dic[pre_field_key] = tmp_pre_field

          tchunk = deepcopy(chunk)
          tchunk.crop(extra_off+pad, mip)

          args_list = [[tmp_profile_cv, z, dic, pre_field_key, mip, tchunk,
                        chunk_size, start_coor],
                       [dic, img_key, tmp_img_cv, z, tchunk, mip, True],
                       [dic, field_key, tmp_vvote_field_cv, z, tchunk, mip,
                        chunk_size]]
          ...
          p.start()
          self.p_list.append(p) # if read partial need to wait
    ...
    while(has_next):
        ...
        for i in range(vvote_way):
            for g in range(num_patch):
                ...
                vv_fields[g,i,...]= field_list[i][:,
                                                  xs*chunk_size:xs*chunk_size+chunk_size,
                                                  ys*chunk_size:ys*chunk_size+chunk_size,
                                                  :]
        ...
        for i in range(num_patch):
            ...
            dst_field[:,xs*chunk_size:xs*chunk_size+chunk_size,
                        ys*chunk_size:ys*chunk_size+chunk_size,:]=new_field[i:i+1,...]
    return dst_field
...
# comment out all crop_chunk since loading the whole image
  ...
  def process_super_chunk_serial(self, src, block_start, serial_range,
                                 ...
                                 schunk, mip, pad, chunk_size,
                                 write_chunk, finish_dir, extra_off,
                                 head_crop_len, end_crop_len, final,
      ...
      chunk.uncrop(extra_off, mip=mip)
      final_chunk =deepcopy(write_chunk)
      final_chunk.crop(pad, mip)
      dic = self.dic
      ...
      for j, z in enumerate(serial_range[serial_range_start:]):
          ...
          dst_field = self.new_compute_field_multi_GPU(model_path, src_image,
                                                       ...
                                                       pad, extra_off, mip)
          new_image = self.warp_slice(chunk_size, pad, mip, src_image,
                                          dst_field, offset=extra_off)
          image_list.append(new_image)
          #print("image size is ", new_image.shape)
          img_key = "store_img" + str(z)
          print("-----------head_crop_len is ", head_crop_len, "end_crop_len is",
                end_crop_len)
          dic[img_key] = new_image[...,pad+head_crop_len:-(pad+end_crop_len),
                                   pad:-pad]
          #print("image after crop shape is ", dic[img_key].shape)
  ...
  def process_super_chunk_vvote(self, src, block_range, max_radius,
                                ...
                                vvote_field, finish_dir, pre_field_cv,
                                start_coor,
                                head_crop=False, end_crop=False, final=True,
                                mask_cv=None,
      ...
      for z in pre_field_range:
          profile_chunk = deepcopy(schunk)
          profiel_chunk.crop(pad, mip)
          pre_field_key = self.pre_field_path+str(mip)+"/"+str(z)
          p = Process(target=self.read_pre_field, args=(pre_field_cv,
                                                        z, dic,
                                                        pre_field_key,
                                                        profile_chunk,
                                                        start_coor,
      ...
      #f_list = [self.mp_store_img, self.mp_store_field, self.write_pre_field]
      f_list = [self.write_pre_field, self.mp_store_img, self.mp_store_field]
      if start_index+super_chunk_len > len(block_range):
          vv_range = block_range[start_index:]
      else:
          vv_range = block_range[start_index:start_index+super_chunk_len]

      print("range in this pass", vv_range)
      for j, z in enumerate(vv_range):
          print("===========z is ", z)
          dst = dsts
          model_path = model_lookup[z]
          chunk = self.crop_chunk(chunk, mip, head_crop_len, end_crop_len, 0, 0)
          ...
          #print("before crop image list , image shape")
          #for i in image_list:
          #    print(i.shape)
          x_len = src_image.shape[-2] - 2*extra_off
          self.crop_image_list(head_crop, end_crop, image_list, x_len)
          #print("after crop image list , image shape")
          #for i in image_list:
          #    print(i.shape)
          self.crop_prefield_list(head_crop, end_crop, pre_field)
          dst_field = self.new_vector_vote(model_path, src_image, image_list,
          ...
          print(" image size ", dic[img_key].shape)
          field_key ="field"+str(z)
          dic[field_key] = dst_field[:,pad+head_crop_len:-(pad+end_crop_len),
                                     pad:-pad,:]
          ...
          args_list=[[pre_field_cv, z, dic, pre_field_key, mip, tchunk,
                      chunk_size, start_coor],
                     [dic, img_key, dst, z, tchunk, mip, True],
                     [dic, field_key, vvote_field, z, tchunk, mip, chunk_size]
                    ]
  ...
  def crop_image(self, image, head_crop, end_crop, diff):
      if diff == 0:
          return image
      if head_crop and end_crop:
          crop_amount = diff//2
          image = image[..., crop_amount:-crop_amount, :]
      elif head_crop or end_crop:
          if head_crop:
              image = image[..., diff:, :]
          else:
              image = image[..., :-diff, :]
  ...
  def crop_image_list(self, head_crop, end_crop, image_list, x_len):
      for i, img in enumerate(image_list):
          diff = img.shape[-2] - x_len
          image_list[i] = self.crop_image(img, head_crop, end_crop, diff)

  def crop_prefield_list(self, head_crop, end_crop, pre_field_list):
      if not isinstance(pre_field_list[-1], torch.Tensor):
          return
      if not isinstance(pre_field_list[-2], torch.Tensor):
          return

      x_len = pre_field_list[-1].shape[0]
      for i, p in enumerate(pre_field_list[:-1]):
          if not isinstance(p, torch.Tensor):
              continue
          diff = p.shape[0]-x_len
          if diff ==0:
              continue
          if head_crop and end_crop:
              crop_amount = diff//2
              p = p[crop_amount:-crop_amount,...]
          elif head_crop or end_crop:
              if head_crop:
                  p = p[diff:,...]
              else:
  ...
  def crop_field_list(self, head_crop, end_crop, field_list):

      x_len = field_list[-1].shape[1]
      for i, p in enumerate(field_list[:-1]):
          if not isinstance(p, torch.Tensor):
              continue
          diff = p.shape[1]-x_len
          if diff ==0:
              continue
          if head_crop and end_crop:
              crop_amount = diff//2
        ...
        for i in range(vvote_way):
            for j in range(num_patch):
                ...
                if  not isinstance(pre_field[vv_off+i], torch.Tensor):
                    ...
                    #print("pre_field shape", pre_field[vv_off+i].shape)
                    #print("xs, ys, ", xs, ys)
