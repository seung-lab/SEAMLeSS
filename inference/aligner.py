...
class Aligner:
  ...
  def mask_threshold_chunk(self, threshold, op, cv_list, z_list, bbox, mip_list, dst_mip):
      mask = None
      for cv, z, mip in zip(cv_list, z_list, mip_list):
        data_in = self.get_data(cv, z, bbox, src_mip=mip,
                                 dst_mip=dst_mip, to_float=False,
                                 to_tensor=False)
        if op == 'gt':
          filtered_data = data_in > threshold
        elif op == 'lt':
          filtered_data = data_in < threshold
        elif op == 'lte':
          filtered_data = data_in <= threshold
          ...
          raise Exception("Unsupported op: '{}' for a mask threshold operation".format(op))

        if mask is None:
          mask = filtered_data
        else:
          mask = np.logical_and(mask, filtered_data)
