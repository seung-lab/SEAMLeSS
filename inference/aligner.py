...
class Aligner:
    def __init__(self, model_path, max_displacement, crop,
        ...
        self.cumres_ng_paths = [os.path.join(dst_ng_path, 'cumulative_vec/{}'.format(i))
                                for i in range(self.process_high_mip + 10)]  # TODO
        self.x_cumres_ng_paths = [os.path.join(
            r, 'x') for r in self.cumres_ng_paths]
        self.y_cumres_ng_paths = [os.path.join(
            r, 'y') for r in self.cumres_ng_paths]

        self.resup_ng_paths = [os.path.join(dst_ng_path, 'vec_up/{}'.format(i))
                               for i in range(self.process_high_mip + 10)]  # TODO
        self.x_resup_ng_paths = [os.path.join(
            r, 'x') for r in self.resup_ng_paths]
        self.y_resup_ng_paths = [os.path.join(
            r, 'y') for r in self.resup_ng_paths]

        self.cumresup_ng_paths = [os.path.join(dst_ng_path, 'cumulative_vec_up/{}'.format(i))
                                  for i in range(self.process_high_mip + 10)]  # TODO
        self.x_cumresup_ng_paths = [os.path.join(
            r, 'x') for r in self.cumresup_ng_paths]
        self.y_cumresup_ng_paths = [os.path.join(
            r, 'y') for r in self.cumresup_ng_paths]

        self.field_ng_paths = [os.path.join(dst_ng_path, 'field/{}'.format(i))
                               for i in range(self.process_high_mip + 10)]  # TODO
        self.x_field_ng_paths = [os.path.join(
            r, 'x') for r in self.field_ng_paths]
        self.y_field_ng_paths = [os.path.join(
            r, 'y') for r in self.field_ng_paths]

        self.net = Process(model_path, mip_range[0], is_Xmas=is_Xmas, cuda=True, dim=high_mip_chunk[0]+crop*2,
                           skip=skip, topskip=topskip, size=size, flip_average=flip_average, old_upsample=old_upsample)
    ...
    def _create_info_files(self, max_offset):
        ...
        if dst_size_increase % chunk_size != 0:
            dst_size_increase = dst_size_increase - \
                (dst_size_increase % max_offset) + chunk_size
    ...
    def compute_residual_patch(self, source_z, target_z, out_patch_bbox, mip):
        print("Computing residual for region {}.".format(
            out_patch_bbox.__str__(mip=0)), flush=True)
        precrop_patch_bbox = deepcopy(out_patch_bbox)
        precrop_patch_bbox.uncrop(self.crop_amount, mip=mip)

        if mip == self.process_high_mip:
            src_patch = self.get_image_data(
                self.src_ng_path, source_z, precrop_patch_bbox, mip)
        else:
            src_patch = self.get_image_data(
                self.tmp_ng_path, source_z, precrop_patch_bbox, mip)

        if (self.run_pairs):
             # only align consecutive pairs of source slices TODO: write function to compse resulting vector fields
            tgt_patch = self.get_image_data(
                self.src_ng_path, target_z, precrop_patch_bbox, mip, should_backtrack=True)
        else:
            # align to the newly aligned previous slice
            tgt_patch = self.get_image_data(
                self.dst_ng_path, target_z, precrop_patch_bbox, mip, should_backtrack=True)
        field, residuals, encodings, cum_residuals = self.net.process(
            src_patch, tgt_patch, mip, crop=self.crop_amount, old_vectors=self.old_vectors)
        ...
        self.save_vector_patch(
            field, self.x_field_ng_paths[mip], self.y_field_ng_paths[mip], source_z, out_patch_bbox, mip)

        if self.write_intermediaries:

            mip_range = range(self.process_low_mip+self.size -
                              1, self.process_low_mip-1, -1)
            for res_mip, res, cumres in zip(mip_range, residuals[1:], cum_residuals[1:]):
                ...
                if self.upsample_residuals:
                    ...
                    res = self.scale_residuals(
                        res, res_mip, self.process_low_mip)
                    ...
                    cumres = self.scale_residuals(
                        cumres, res_mip, self.process_low_mip)
            ...
            for k, enc in enumerate(encodings):
                ...
                # dst_image_encodings
                write_encodings(
                    slice(enc.shape[-1] // 2, enc.shape[-1]), target_z)
    ...
    def warp_patch(self, ng_path, z, bbox, res_mip_range, mip, start_z):
        influence_bbox = deepcopy(bbox)
        influence_bbox.uncrop(self.max_displacement, mip=0)
        start = time()

        agg_flow = self.get_aggregate_rel_flow(
            z, influence_bbox, res_mip_range, mip)  # download  vect
    ...
    def scale_residuals(self, res, src_mip, dst_mip):
        print('Upsampling residuals from MIP {0} to {1}'.format(
            src_mip, dst_mip))
    ...
    def save_residual_patch(self, res, crop, x_path, y_path, z, bbox, mip):
        print("Saving residual patch {} at MIP {}".format(
            bbox.__str__(mip=0), mip))
    ...
    def missing_data_mask(self, img, bbox, mip):
        (img_xs, img_xe), (img_ys, img_ye) = bbox.x_range(
            mip=mip), bbox.y_range(mip=mip)
        (total_xs, total_xe), (total_ys, total_ye) = self.total_bbox.x_range(
            mip=mip), self.total_bbox.y_range(mip=mip)
            ...
            for key in self.img_cache:
                ...
                if other_mip == mip and other_path == path:
                    if bbox.intersects(other_bbox):
                        ...
                        output[:, :, xs:xs+xsz, ys:ys +
                               ysz] = self.img_cache[key]
        if np.min(output > -1):
            print('hit')
            return output
        else:
    ...
    def get_image_data(self, path, z, bbox, mip, should_backtrack=False):
        ...
        if self.num_targets > 1 and should_backtrack:
            for backtrack in range(1, self.num_targets):
                ...
                self.supplement_target_with_backup(
                    data, still_missing_mask, backup, bbox, mip)
    ...
    def get_abs_residual(self, z, bbox, mip):
        x = self.get_vector_data(
            self.x_field_ng_paths[mip], z, bbox, mip)[..., 0, 0]
        y = self.get_vector_data(
            self.y_field_ng_paths[mip], z, bbox, mip)[..., 0, 0]
    ...
    def get_field_sf_residual(self, z, bbox, mip):
        x_range = bbox.x_range(mip=mip)
        y_range = bbox.y_range(mip=mip)
        field_sf = cv(self.field_sf_ng_path, mip=mip, bounded=False, fill_missing=True, progress=False)[
            x_range[0]:x_range[1], y_range[0]:y_range[1], z]
        ...
        # print("abs_res shape: ", abs_res.shape)
        rel_res = self.abs_to_rel_residual(abs_res, bbox, mip)
        return rel_res
    ...
    def get_aggregate_rel_flow(self, z, bbox, res_mip_range, mip):
        result = torch.zeros(
            (1, bbox.x_size(mip), bbox.y_size(mip), 2), dtype=torch.float)
        ...
        for res_mip in range(start_mip, end_mip + 1):
            ...
            up_rel_res = upsample(
                res_mip - mip)(rel_res.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            result += up_rel_res
    ...
    def copy_section(self, source, dest, z, bbox, mip):
        print("moving section {} mip {} to dest".format(
            z, mip), end='', flush=True)
        start = time()
    ...
    def copy_field(self, z, bbox, mip):
        ...
        def chunkwise(patch_bbox):
            ...
            influence_bbox.uncrop(self.max_displacement, mip=0)
            field_sf = self.get_rel_residual(z, influence_bbox, mip)
            self.save_field_patch(field_sf, influence_bbox, mip, z)
        self.pool.map(chunkwise, chunks)
        print("finish copy field")

    def prepare_source(self, z, bbox, mip, start_z=-1):
        ...
        def chunkwise(patch_bbox):
            warped_patch = self.warp_patch(self.src_ng_path, z, patch_bbox,
                                           (mip + 1, self.process_high_mip), mip, start_z)
            self.save_image_patch(
                self.tmp_ng_path, warped_patch, z, patch_bbox, mip)
        self.pool.map(chunkwise, chunks)
        end = time()
        print(": {} sec".format(end - start))
    ...
    def downsample(self, z, bbox, source_mip, target_mip):
        print("Downsampling {} from mip {} to mip {}".format(
            bbox.__str__(mip=0), source_mip, target_mip))
            ...
            def chunkwise(patch_bbox):
                ...
                if (self.run_pairs):
                    downsampled_patch = self.downsample_patch(
                        self.dst_ng_path, z-1, patch_bbox, m)
                    self.save_image_patch(
                        self.dst_ng_path, downsampled_patch, z-1, patch_bbox, m)
                else:
                    downsampled_patch = self.downsample_patch(
                        self.dst_ng_path, z, patch_bbox, m)
                    self.save_image_patch(
                        self.dst_ng_path, downsampled_patch, z, patch_bbox, m)
            self.pool.map(chunkwise, chunks)
    ...
    def compute_section_pair_residuals(self, source_z, target_z, bbox):
        for m in range(self.process_high_mip,  self.process_low_mip - 1, -1):
            ...
            def chunkwise(patch_bbox):
                ...
                self.compute_residual_patch(
                    source_z, target_z, patch_bbox, mip=m)
            self.pool.map(chunkwise, chunks)
            end = time()
            print(": {} sec".format(end - start))
        ...
        if move_anchor:
            for m in range(self.render_low_mip, self.high_mip+1):
                self.copy_section(self.src_ng_path,
                                  self.dst_ng_path, start_section, bbox, mip=m)
