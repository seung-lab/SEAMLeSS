...
class BoundingBox3d:
  def __init__(self, xs, xe, ys, ye, zs, ze, mip, max_mip=12):
    self.max_mip = max_mip
    scale_factor = 2**mip
    self.set_m0(xs*scale_factor, xe*scale_factor, ys*scale_factor, ye*scale_factor, zs*scale_factor, ze*scale_factor)
  ...
  def insets(self, other, mip):
    ...
    xs, xe = self.x_range(mip)
    ys, ye = self.y_range(mip)
    zs, ze = self.z_range(mip)
    oxs, oxe = other.x_range(mip)
    oys, oye = other.y_range(mip)
    ozs, oze = other.z_range(mip)
  ...
  def get_offset(self, mip=0):
    ...
    return (self.m0_x[0] / scale_factor + self.m0_x_size / 2 / scale_factor,
            self.m0_y[0] / scale_factor + self.m0_y_size / 2 / scale_factor,
            self.m0_z[0] / scale_factor + self.m0_z_size / 2 / scale_factor)
  ...
  def z_range(self, mip):
    assert(mip <= self.max_mip)
    scale_factor = 2**mip
    zs = floor(self.m0_z[0] / scale_factor)
    ze = ceil(self.m0_z[1] / scale_factor)
    return (zs, ze)
  ...
  def __str__(self, mip=0):
    return "{}, {}, {}".format(self.x_range(mip), self.y_range(mip), self.z_range(mip))
  ...
  def stringify(self, mip=0):
    ...
    y_start = self.y_range(mip)[0]    
    y_stop = self.y_range(mip)[1]
    z_start = self.z_range(mip)[0]
    z_stop = self.z_range(mip)[1]    
