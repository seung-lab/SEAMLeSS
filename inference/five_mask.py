...
if __name__ == '__main__':
  ...
  parser.add_argument('--trans_mask_path', type=str)
  parser.add_argument('--slip_mask_path', type=str)
  parser.add_argument('--slip2_mask_path', type=str)
  ...
  parser.add_argument('--trans_mask_mip', type=int)
  parser.add_argument('--slip_mask_mip', type=int)
  parser.add_argument('--slip2_mask_mip', type=int)
  ...
  # Simplify var names
  trans_mip = args.trans_mask_mip
  fold_mip = args.fold_mask_mip
  slip_mip = args.slip_mask_mip
  slip2_mip = args.slip2_mask_mip
  ...
  trans_cv = cm.create(args.trans_mask_path, data_type='uint8', num_channels=1,
                     fill_missing=True, overwrite=False)
  
  slip_cv = cm.create(args.slip_mask_path, data_type='uint8', num_channels=1,
                     fill_missing=True, overwrite=False)

  slip2_cv = cm.create(args.slip2_mask_path, data_type='uint8', num_channels=1,
                     fill_missing=True, overwrite=False) 
  ...
  class TaskIterator():
      ...
      def __iter__(self):
          for z in self.brange:
              t = a.five_mask_op(cm, bbox, fold_cv.path, slip_cv.path, 
                                  tissue_cv.path, dst.path, z, z, z, z, 
                                  fold_mip, slip_mip, tissue_mip,
                                  slip2_cv.path, slip2_mip, trans_cv,
                                  trans_mip)
              yield from t
