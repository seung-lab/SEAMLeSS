...
#  # field need to in float since all are relative value
  ...
  for vvote_start in vvote_subblock:
      ...
      for j in range(len(chunk_grid)):
          ...
          #@asyncio.coroutine
          async def read_data_test(i):
              print("start asyncio ", src.path)
              cv = CloudVolume(src.path, mip=mip, fill_missing=True)
              x_range = chunk.x_range(mip=mip)
              y_range = chunk.y_range(mip=mip)
              image = await cv[x_range[0]:x_range[1],y_range[0]:y_range[1],block_start+vvote_start-i]
              #image_for_vv = 
              #yield from a.load_part_image(src,
              ...
              #r  = yield from a.save_image(im, dst, block_start+vvote_start-i,
              #         final_chunk, mip, to_uint8=False)
              print("end asyncio")
 
          loop = asyncio.get_event_loop()
          tasks = [read_data_test(i) for i in range(vvote_way)]
