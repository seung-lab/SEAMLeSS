...
#      batch.extend(t)
#    ge = time()
#    print('Scheduling ComputeFieldTasks', 'generating use', ge-gs)
#    start = time()
#    run(a, batch)
#    end = time()
#    diff = end - start
...
#
  ...
  for block_offset in vvote_range:
    ...
    prefix = block_offset
    #block_range
    def make_some_tasks(block_r):
        for i, block_start in enumerate(block_r):
          block_type = block_types[i % 2]
          dst = dsts[block_type]
          z = block_start + block_offset 
          bbox = bbox_lookup[z]
          ...
          for z_offset in vvote_offsets:
            ...
            t = a.compute_field(cm, model_path, src.path, dst.path, field.path, 
                                z, z+z_offset, bbox, mip, pad, src_mask_cv=src_mask_cv,
                                src_mask_mip=src_mask_mip, src_mask_val=src_mask_val,
                                tgt_mask_cv=src_mask_cv, tgt_mask_mip=src_mask_mip, 
                                tgt_mask_val=src_mask_val, prefix=prefix)
    ...
    def remote_upload(br):
        #ptasks = make_some_tasks(brange)
        with GreenTaskQueue(queue_name='deepalign_zhen') as tq:
            tq.insert_all(br, total=1000000)
    ge = time()
    print('\nScheduling ComputeFieldTasks, generating use', ge-gs)

    start = time()
#    run(a, make_some_tasks(), total=100000)
#    with ProcessPoolExecutor(max_workers=part) as executor:
#        executor.map(remote_upload, br)
    ...
    for i in range(part-1):
        br.append(TaskIterator(block_range[i*bh:(i+1)*bh])) 
    br.append(TaskIterator(block_range[(part-1)*bh:-1]))
    tasks = scatter(br, 4)
    #remote_upload(TaskIterator(block_range))
    with ProcessPoolExecutor(max_workers=part) as executor:
        executor.map(remote_upload, br)
    print("after sending .....")
    end = time()
...
#      batch.extend(t)
#    ge = time()
#    print('\nScheduling VectorVoteTasks, generating time', ge-gs)
#    start = time()
#    run(a, batch)
#    end = time()
#    diff = end - start
...
#      task_counter[k] += len(t)
#    ge = time()
#    print('\nScheduling RenderTasks, generating time', ge-gs)
#    start = time()
#    run(a, batch)
#    end = time()
#    diff = end - start
