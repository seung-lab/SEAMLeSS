...
#  # field need to in float since all are relative value
  ...
  class WriteImage(object):
      ...
      def __iter__(self):
          for i in self.brange:
              t =a.store_random_image(src.path, dst.path, range(i,i+1), chunk_grid[0], mip,
                                             args.block_size, pad, final_chunk, compress=False) 
              yield from t
  #batch = a.store_random_image(src.path, dst.path, z_range, chunk_grid[0], mip,
  #                                 args.block_size, pad, final_chunk, compress=False)
  ...
  for irange in range_list:
      ptask.append(WriteImage(irange))
