...
from utilities.helpers import make_range, print_run, ranges_overlap

from task_scheduling import TaskExecutor


def get_stage_cloudvolumes(stage_params):
    cm = CloudManager(
        args.src_path,
        max_mip,
        pad,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--param_lookup",
        type=str,
        help="relative path to YAML file identifying params to use per z range",
    )
    ...
    a = Alginer(threads=args.threads, args.queue_name,
                task_batch_size=args.task_batch_size)
                device=args.device, dry_run=args.dry_run)
    provenance = get_provenance(args)

    run_params = yaml.load(open(args.param_lookup))
    max_mip =
    restart =
    skip_list = run_params['skip_list']
    bbox =
    dst =

    chunk_size = run_params['']
