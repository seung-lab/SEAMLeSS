...
import gen_tasks as mt
from tasks import green_upload
...
if __name__ == '__main__':
  ...
  # Copy first section
  #mt.make_copy_tasks(a, copy_range, block_range, block_types, bbox_lookup, cm,
  #                    src, dsts, mip, src_mask_cv, src_mask_mip, src_mask_val)

  ##mt.test_parallel_insert_all() 
...
#  task_counter = {}
  for block_offset in copy_range:
    ...
    for i, block_start in enumerate(block_range):
      ...
      bbox = bbox_lookup[z]
      ptask = a.copy(cm, src.path, dst.path, z, z, bbox, mip, is_field=False, mask_cv=src_mask_cv,
                     mask_mip=src_mask_mip,
                     mask_val=src_mask_val,prefix=prefix, return_iterator=True) 
      green_upload(ptask, a)
   # tq = GreenTaskQueue('deepalign_zhen')
   # tq.insert_all(ptask, parallel=2) 
      #k = (prefix, dst.path)
...
#      #batch.extend(t)
#      green_upload(a.queue_name, t, a.threads)
#
...
#                   prefix=prefix, return_iterator=True)
#      green_upload(a.queue_name, t, a.threads)
#      k = (prefix, dst.path)
#      if k not in task_counter:
#        task_counter[k] = 0
   ...
   # batch = []
    for block_start in block_range:
      ...
      t = a.vector_vote(cm, pair_fields, vvote_field, z, bbox, mip, inverse=False, 
                        serial=True, prefix=prefix, return_iterator=True)
      green_upload(a.queue_name, t, a.threads)
      #batch.extend(t)
