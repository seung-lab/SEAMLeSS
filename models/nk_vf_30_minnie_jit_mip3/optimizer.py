...
def optimize_pre_post_multiscale_ups(pred_res_start, src, tgt, src_defects, tgt_defects, mips, crop=2, bot_mip=4, img_mip=4, max_iter=800):
    ...
    opti_losses = {
        3: unsupervised_loss(smoothness_factor=sm[3], use_defect_mask=True,
    ...
    for m in mips:
        ...
        for i in range (bot_mip - img_mip):
            ...
            src_downs = torch.nn.functional.max_pool2d(src_downs, 2)
            tgt_downs = torch.nn.functional.max_pool2d(tgt_downs, 2)
...
def optimize(src, tgt, pred_res_start, src_defects, tgt_defects, img_mip=3, max_iter=400,
        normalize_imgs=False):
    mips = [6, 5, 4, 3]
    if normalize_imgs:
        src = normalize(src, bad_mask=src_defects)
        tgt = normalize(tgt, bad_mask=tgt_defects)

    src_defects = src_defects.squeeze(0)
    tgt_defects = tgt_defects.squeeze(0)
    pred_res_opt = optimize_pre_post_multiscale_ups(pred_res_start, src, tgt, src_defects, tgt_defects, mips, crop=0, bot_mip=3, img_mip=3, max_iter=max_iter)
    return pred_res_opt
