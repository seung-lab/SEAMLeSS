...
class SelfSupervisedLoss(nn.Module):
    ...
    def forward(self, sample, prediction, tgt_field=None, fine_prediction=None):
        prediction.field_()
        ...
        if fine_prediction is not None:
            # only look at difference between fine and coarse
            field_loss_map = self.field_penalty([fine_prediction.permute(0, 2, 3, 1)])
        else:
