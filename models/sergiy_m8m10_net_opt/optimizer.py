...
def optimizer(src, tgt, pred_res, src_folds):
    ...
    src = src.squeeze(0)
    tgt = tgt.squeeze(0)
    pred_res = pred_res.squeeze(0)
    src_folds = src_folds.squeeze(0)
    loss_bundle = {
            'src': src,
            'tgt': tgt,
            'pred_res': pred_res,
            'pred_tgt': res_warp_img(src, pred_res, is_pix_res=True),
            'src_defects': src_folds,
            'src_edges': torch.zeros_like(src),
            'src_plastic': torch.zeros_like(src),
            'tgt_plastic': torch.zeros_like(src),
