...
def block_match(tgt, src, tile_size=16, tile_step=16, max_disp=10, min_overlap_px=500, min_overlap_ratio=0.5,
                filler="inf", output_tile_size=32, cutoff_peak_ratio=2.5):
    src = src.squeeze()
    tgt = tgt.squeeze()

    tile_alignment_pad = 0

    if True:
        tile_alignment_pad = (tile_size - tile_step) // 2
        tgt = torch.nn.ZeroPad2d(tile_alignment_pad)(tgt)
        src = torch.nn.ZeroPad2d(tile_alignment_pad)(src)

    img_size = tgt.shape[-2]
    tile_count = 1 + (img_size - tile_size) // tile_step
    #assert (img_size % output_tile_size == 0)

    #tile_count = img_size // output_tile_size
    #tile_size = img_size - tile_step * (tile_count - 1)
    #print (tile_size)
    #import pdb; pdb.set_trace()
    max_disp_pad = max_disp
    padded_tgt = torch.nn.ZeroPad2d(max_disp_pad)(tgt)
    padded_src = torch.nn.ZeroPad2d(max_disp_pad)(src)
    img_size = padded_tgt.shape[-2]
        ...
        for y_tile in range(0, tile_count):
            src_tile_coord, tgt_tile_coord = compute_tile_coords(x_tile, y_tile, tile_size,
                                                              tile_step, max_disp, img_size,
                                                                x_offset=max_disp,
                                                                y_offset=max_disp)
            #print (src_tile_coord)
            src_tile = padded_src[src_tile_coord]
            tgt_tile = padded_tgt[tgt_tile_coord]
            src_black_frac = get_black_fraction(src_tile, 0)
            tgt_black_frac = get_black_fraction(tgt_tile, 0)
            if src_black_frac > 0.98 or tgt_black_frac > 0.98:
                match_displacement = [0, 0]
                if src_black_frac != 1.0 and \
                        tgt_black_frac != 1.0:
                    #print ('skipping: {} {}'.format(src_black_frac, tgt_black_frac))
                    pass
            else:
                if tgt_tile.shape[0] != tgt_tile.shape[1]:
                    src_tile_coord, tgt_tile_coord = compute_tile_coords(x_tile, y_tile, tile_size,
                                                              tile_step, max_disp, img_size,
                                                                x_offset=max_disp,
                                                                y_offset=max_disp)
                ...
                if ncc.var() < 1E-17 or ((ncc != ncc).sum() > 0):
                    #match_displacement = [0, 0]
                    ...
                    peak_ratios.append(peak_vals[-1][0] / (peak_vals[-1][1] + 1e-19))
                    if peak_ratios[-1] < cutoff_peak_ratio:
                        match_displacement = [float(filler), float(filler)]
                    else:
    ...
    result_ups_var = torch.nn.functional.pad(result_ups_var,
            (-tile_alignment_pad, -tile_alignment_pad, -tile_alignment_pad, -tile_alignment_pad))
...
def get_ncc(tgt, tmpl, div_by_overlap=False, min_overlap_ratio=0.7,
    ...
    peak = get_peak(ncc)
    peak_overlap = overlap_ratio[..., peak[0][0], peak[0][1]]
    #print (peak_overlap)
    if peak_overlap > 1.0E-4 and peak_overlap < min_overlap_ratio:
        ncc[...] = 0
        #print ("GONE")
