...
def coarsen_mask_opt(mask, n=1, flip=True):
    ...
    for settings in keys_to_apply:
        ...
        if name_in_bundle != None:
            ...
            if mask_value_in_settings != None:
                prewarp_result = torch.where(mask != 1.0, torch.tensor(mask_value_in_settings, device=prewarp_result.device, dtype=prewarp_result.dtype), prewarp_result)
            else:
                prewarp_result = torch.where(mask != 1.0, torch.zeros_like(prewarp_result), prewarp_result)
            ...
            if coarsen_ranges_in_settings != None:
                ...
                for length, weight in coarsen_ranges_in_settings:
                    if length > 10:
                        around_the_mask = coarsen_mask_opt(around_the_mask, length)
                    else:
                        around_the_mask = coarsen_mask_opt(around_the_mask, length)
...
def warp_prewarp_mask(prewarp_result, res, do_nothing=False, inv=False):
    if do_nothing:
        ...
        if (res != 0).sum() > 0 and not inv:
            if res.shape[1] == 2:
                result = res_warp_img(prewarp_result.float(), res, is_pix_res = False)
            else:
...
def get_mse_and_smoothness_masks_opt(bundle, mse_keys_to_apply,
    ...
    return get_warped_srctgt_mask_opt(bundle, mse_keys_to_apply, sm_keys_to_apply)

def get_warped_srctgt_mask_opt(bundle, mse_keys_to_apply, sm_keys_to_apply):
